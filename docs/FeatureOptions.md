...
| <A NAME="Nvr.Reboot"></A><CODE>Nvr.Reboot<I>.Value</I></CODE>                       | Proactively reboot the UniFi Protect controller on a scheduled interval, in hours. Active HomeKit Secure Video recording events will be allowed to complete before rebooting. Use with care. **(default: 6)**. <BR>*Supported on Protect controllers.*
...
| <A NAME="Video.Stream.AlwaysPrebuffer"></A>`Video.Stream.AlwaysPrebuffer`           | Maintain a continuous live-view prebuffer via the Protect livestream API, independent of HKSV. Speeds up rapid HomeKit live-view opens by eliminating RTSP session re-establishment, at the cost of a persistent livestream connection per camera. **(default: disabled)**.
