...
#### Configuring HKSV For UniFi Protect
  ...
  * On a technical level, HKSV rides the standing *timeshift buffer* that HBUP already keeps for every camera...the same buffer that powers your live views and snapshots. (See [How HBUP Sources Your Video](https://github.com/hjdhjd/homebridge-unifi-protect/blob/main/docs/Autoconfiguration.md#how-hbup-sources-your-video) for the full picture.) That buffer is fed by the UniFi Protect livestream API, which isn't quite the same thing as RTSP...it's an **actual UniFi Protect API** that gives direct access to the raw livestream on the Protect controller. The buffer is only a few seconds long - in practice, HomeKit seems to always ask for the preceding four seconds. Think of it as a timeshifting DVR constantly holding a small rolling window of live video: when a motion event fires, HKSV consumes that look-back and keeps recording for as long as HomeKit wants it. Feeding HKSV from the livestream API this way significantly lowers the load on the Protect controller, particularly once you get beyond a few cameras, and on the machine running HBUP, since it avoids the overhead of pulling a separate RTSP stream through FFmpeg. Because the buffer is shared, HKSV and snapshots keep using it even if you've switched your live views to direct RTSP.
...
UniFi Protect has its own smart motion detection capabilities that [can be used](https://github.com/hjdhjd/homebridge-unifi-protect/blob/main/docs/BestPractices.md#motion-sensors-motion-switches-and-push-notifications) with `homebridge-unifi-protect`. When you have smart motion detection enabled and you have HomeKit Secure Video enabled **and** configured in the Home app to record events, `homebridge-unifi-protect` is faced with a dilemma: when do I notify a user of a motion detection event?
...
However, if you have the smart motion detection object sensors feature option enabled, you will still receive contact sensor updates for those object types as UniFi Protect detects them. It's important to note: smart object contact sensors are not related, nor connected, to HKSV. HKSV is a bit of a black box and handles everything independently, without our knowledge or ability to know what it's detected, by design. It's entirely possible that the smart object sensors will detect (or not detect) a certain object that HKSV does or doesn't detect.

#### How HomeKit Secure Video Works
Apple created an elegant secure-by-default system that leverages its home-based devices such as HomePods and Apple TVs for the heavy lifting of analyzing video to create the HKSV user experience.
  ...
  * That recorded video event is then sent to iCloud in an encrypted form, and the alert notifications are sent to your iPhone, etc., based on whatever your notification settings allow.
...
**The recommended *ideal* hardware for running HBUP for use with HKSV is an Apple Silicon-based Mac with hardware acceleration enabled within HBUP. Why not RPi 4 or other hardware? Raspberry Pi has decent (not great) hardware acceleration, but critically, very bug-prone software and APIs to access those hardware features, at least when it comes to FFmpeg. Intel machines fare a bit better. Macs have great hardware *and* software support for hardware acceleration, especially with FFmpeg. Apple Silicon goes one step further by adding an additional hardware enhancement to Apple's already good hardware acceleration that further improves the quality of transcoded video. Run Homebridge natively on macOS, not using Docker or another containerization solution, due to hardware acceleration being unavailable to containers.**

#### Performance Considerations
HKSV in `homebridge-unifi-protect` works well by default if you have a decently performant and modern machine that you're running Homebridge on. The price you pay for having HKSV comes primarily in the area of CPU. HBUP *will* need to transcode any video it sends to a Home hub for analysis. **The good and bad news is that it only happens when motion is detected, which triggers an analysis cycle by the Home hub. If you have a lot of cameras and a lot of motion events, expect this to happen often, along with the requisite consumption of CPU.**

If you're struggling to get HKSV working in HBUP, try the following and see if it helps:

  * Force the timeshift buffer that feeds HKSV to use the lowest-quality stream as its starting point. You'll find this under the *Timeshift Buffer* section of the HBUP feature options webUI (`Video.Timeshift.Only.Low`). Because that buffer is shared, this also lowers the quality of your live views and snapshots, not just your HKSV recordings.
...
#### Things To Be Aware Of
  * It's strongly recommended that the local UniFi Protect account you use with `homebridge-unifi-protect` has the full management role. Without it, HBUP can't auto-enable RTSP streams for you, so you'll need to enable at least one RTSP stream on each camera yourself...HKSV depends on having a stream to record from.
  ...
  * Recordings are resilient to brief controller and livestream hiccups...a momentary stall won't end the event and cost you the clip.
