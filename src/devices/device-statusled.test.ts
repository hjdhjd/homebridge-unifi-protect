 ...
 * lightDeviceSettings command) is LIGHT-family-specific and stays owned by light.test.ts; the chime / viewer projections expose no status indicator and never configure
 * the switch, so the statusLed false-branch ("ledSettings" in ufp false) is unreachable via this carrier and is honestly out of this file's reach.
 *
