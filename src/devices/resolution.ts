...
// Compose the SRTP-enabled RTSPS URL for a channel. Pure string composition - the host and port are supplied by the caller (the device owns the host chain), and the
// alias is the channel's own. No I/O.
export function rtspsUrl(channel: ProtectCameraChannelConfig, urlHost: string, rtspPort: number): string {

  return "rtsps://" + urlHost + ":" + rtspPort.toString() + "/" + channel.rtspAlias + "?enableSrtp";
}
...
// classes: the friendly name and resolution come from the channel's native dimensions, the URL from rtspsUrl, and the optional lens is omitted entirely when
...
export function buildChannelProfile(channel: ProtectCameraChannelConfig, options: { lens?: number; rtspPort: number; urlHost: string }): ChannelProfile {
  ...
  const entry: ChannelProfile = {
    ...
    url: rtspsUrl(channel, options.urlHost, options.rtspPort)
  };
