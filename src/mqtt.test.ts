 ...
 * and the event and stream owners - composes through it, so the wire topic stays {topicPrefix}/{id}/{topic} after HBPU's MqttClient prefixes the configured topic. These
 * tests pin that composition directly against the pure function: a single-segment tail, a multi-segment tail (the smart-motion metadata shape), and both id scopes - a
 * device MAC and a controller MAC are just id values to the composer.
 */
