 ...
 * configureMqtt registers (camera.ts:1517-1576), driven against the REAL constructed ProtectCamera by FINDING each captured subscription on the TestMqttClient double and
...
import { G2_PRO_CHANNELS, MIXED_RTSP_DISABLED_CHANNELS } from "./resolution.fixtures.ts";
...
// Build a REAL plain ProtectCamera against the harness doubles, exactly as camera-onsets.test.ts:67-80 assembles it, with two additions this suite needs: mqtt is always
...
describe("camera-leaf MQTT handler bodies (camera-mqtt concern net)", () => {
  ...
  describe("the /rtsp GET (the JSON RTSP URL map)", () => {
    ...
    test("returns the per-channel RTSP URL map composed from connectionHost, the controller RTSP port, and each channel's alias", async () => {
      ...
      // host, and makeNvrConfig seeds ports.rtsp to 7447, so the body composes rtsps://<connectionHost>:<rtsp-port>/<alias>?enableSrtp for each. The URL string is the
      // contract the body promises, so we assert it literally.
      const urls = JSON.parse(rtsp.getValue()) as Record<string, string>;

      assert.deepEqual(urls, {

        High: "rtsps://camera.test:7447/alias0?enableSrtp",
        Low: "rtsps://camera.test:7447/alias2?enableSrtp",
        Medium: "rtsps://camera.test:7447/alias1?enableSrtp"
    ...
    test("the isRtspEnabled filter drops a channel whose RTSP is disabled", async () => {
      ...
      assert.equal(urls["High"], "rtsps://camera.test:7447/alias0?enableSrtp", "the RTSP-enabled High channel is present with its composed URL");
      assert.equal(urls["Low"], "rtsps://camera.test:7447/alias2?enableSrtp", "the RTSP-enabled Low channel is present with its composed URL");
    });
  });
