...
// then an FFmpeg probesize-estimation failure, which is ungated because v1 ran the probesize self-tune for every FFmpeg error regardless of source; otherwise the
// canonical verdict. The benign branch reproduces v1's ffmpegErrorCheck patterns; the order (benign before probesize) is load-bearing because both can match on the same
// teardown. This function returns the protocol VERDICT only - the user-facing label (LIVESTREAM_API_ERROR_MESSAGE) and all I/O stay with the dispatcher that consumes it.
...
// ffmpegErrorCheck), so we reproduce v1's failed-teardown logging order through the overridable logFailedTeardown hook: the override classifies the failure via
