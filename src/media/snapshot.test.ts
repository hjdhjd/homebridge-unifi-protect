...
import type { TestLogEntry } from "../testing.helpers.ts";
...
function buildSnapshot(kind: "empty" | "stale"): { logEntries: TestLogEntry[]; selectCalls: { count: number }; snapshot: ProtectSnapshot } {
  ...
  return { logEntries, selectCalls, snapshot: new ProtectSnapshot(host) };
}
