...
import type { DoorbellCapability } from "./devices/doorbell.ts";
import { G6_PRO_ENTRY_CHANNELS } from "./resolution.fixtures.ts";
...
describe("package camera remove-on-false (BC2)", () => {
  ...
  test("a capability withdrawal with the grace disabled detaches immediately: teardown, guarded tail, event-timer and MQTT hygiene, and silence afterward", async () => {
    ...
    // Post-detach, the package is gone reactively too: only the doorbell's fifteen observers remain and a state push wakes nothing attributed to the package.
    ...
    try {
      ...
      assert.equal(store.observerCount, 15, "only the doorbell's observers survive the detach");
